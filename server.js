...
function parseTorvikGameStats(rows) {
  ...
  const okRow = r => {
    ...
    const d = normalizeDateStr(cl(r[0]));
    const team = cl(r[TEAM]), opp = cl(r[OPP]);
    const tp = parseInt(cl(r[TPTS])), op = parseInt(cl(r[OPTS]));
  ...
  for (const r of rows) {
    if (!okRow(r)) continue;
    const d = normalizeDateStr(cl(r[0])), team = cl(r[TEAM]), opp = cl(r[OPP]);
    const key = d + "|" + [normName(team), normName(opp)].sort().join("|");
    if (seen[key]) continue;
    seen[key] = 1;
    out.push({ d: d, home: team, away: opp, hPts: parseInt(cl(r[TPTS])), aPts: parseInt(cl(r[OPTS])), neutral: false });
  }
