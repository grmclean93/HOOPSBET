...
async function redisProbe() {
  ...
  const key = "hb:__probe", val = "p" + Date.now();
  const ctrl = new AbortController(), timer = setTimeout(function () { ctrl.abort(); }, 4000);
  try {
    const w = await fetch(REDIS_URL + "/SET/" + encodeURIComponent(key) + "/" + encodeURIComponent(val), { headers: { Authorization: "Bearer " + REDIS_TOKEN }, signal: ctrl.signal });
    ...
    const rd = await fetch(REDIS_URL + "/GET/" + encodeURIComponent(key), { headers: { Authorization: "Bearer " + REDIS_TOKEN }, signal: ctrl.signal });
    ...
    return { ok: readBack === val, configured: true, scheme: scheme, error: readBack === val ? null : "round-trip mismatch" };
  } catch (e) {
    return { ok: false, configured: true, scheme: scheme, error: "request failed: " + (e.name === "AbortError" ? "timeout (4s)" : e.message) };
