...
app.post("/api/admin/torvik/tmupload", async (q, r) => {
  ...
  for (const f of files) {
    ...
    try {
      ...
      const text = /\.gz$/i.test(name) ? zlib.gunzipSync(raw).toString("utf8") : raw.toString("utf8");
      ...
      stored.push({ date: ymd, teams: built.list.length });
