...
// barttorvik getgamestats.php box-score CSV: date(0), min(1), team(2), opp(3),
// 14 team box-score cols, teamPTS(18), 14 opp box-score cols, oppPTS(33), then trailing fields.
// The generic detector mis-reads this (many score-like cols), so parse it explicitly and only
// trust it if the fixed layout holds on a strong majority of rows.
function parseTorvikGameStats(rows) {
  if (!rows || rows.length < 100) return null;
  const TEAM = 2, OPP = 3, TPTS = 18, OPTS = 33;
  const okRow = r => {
    if (!r || r.length <= OPTS) return false;
    const d = normalizeDateStr(r[0]);
    const team = String(r[TEAM] == null ? "" : r[TEAM]).trim(), opp = String(r[OPP] == null ? "" : r[OPP]).trim();
    const tp = parseInt(r[TPTS]), op = parseInt(r[OPTS]);
    return /^\d{4}-\d{2}-\d{2}$/.test(d) && !!team && !!opp && isNaN(+team) && isNaN(+opp) && tp >= 20 && tp <= 200 && op >= 20 && op <= 200;
  };
  const sample = rows.filter(r => r && r.length > OPTS).slice(0, 300);
  if (sample.length < 50 || sample.filter(okRow).length < sample.length * 0.8) return null;
  const out = [], seen = {};
  for (const r of rows) {
    if (!okRow(r)) continue;
    const d = normalizeDateStr(r[0]), team = String(r[TEAM]).trim(), opp = String(r[OPP]).trim();
    const key = d + "|" + [normName(team), normName(opp)].sort().join("|");
    if (seen[key]) continue;
    seen[key] = 1;
    out.push({ d: d, home: team, away: opp, hPts: parseInt(r[TPTS]), aPts: parseInt(r[OPTS]), neutral: false });
  }
...
async function fetchTorvikGames(year) {
  for (const mode of ["csv", "json"]) {
    try {
      ...
      if (tg && tg.length >= 100) { await rSet("hb:replay:games:" + year, { games: tg, det: "torvik-getgamestats", mode, fetchedAt: Date.now(), sample: rows.slice(0, 2) }); return { ok: true, games: tg.length, mode, det: "torvik-getgamestats" }; }
